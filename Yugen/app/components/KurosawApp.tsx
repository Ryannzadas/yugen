      ...
      <div className="header-actions">
        ...
        {user && <div className="profile-control" ref={profileMenu}>
          ...
          {profileOpen && <div className="profile-menu" role="menu">
            ...
            <a href="/signout-with-chatgpt?return_to=/" role="menuitem"><span>↪</span> Sair</a>
          </div>}
        </div>}
      </div>
    </header>
...
function AuthModal({ type, onClose, switchTo }: { type: Exclude<Modal, "collection" | "create" | null>; onClose: () => void; switchTo: (modal: Modal) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="modal" onMouseDown={(e) => e.stopPropagation()} role="dialog" aria-modal="true"><button className="modal-close" onClick={onClose} aria-label="Fechar">×</button><p className="eyebrow">Sua biblioteca espera por você</p><h2>{title}</h2>{type !== "forgot" && <><a className="social-button" href="/signin-with-chatgpt?return_to=/profile">◉ Continuar com ChatGPT</a><button className="social-button">G Continuar com Google</button><button className="social-button">● Continuar com Apple</button><div className="or"><span />ou continue com email<span /></div></>}{type === "join" && <label>Nome de usuário<input placeholder="Como as pessoas verão você" /></label>}<label>Email<input type="email" placeholder="voce@exemplo.com" /></label>{type !== "forgot" && <label>Senha<input type="password" placeholder="Pelo menos 12 caracteres" /></label>}<button className="primary-button full">{type === "join" ? "Criar conta" : type === "login" ? "Entrar" : "Enviar link de redefinição"}</button>{type === "login" && <button className="text-button" onClick={() => switchTo("forgot")}>Esqueceu a senha?</button>}<p className="modal-switch">{type === "join" ? "Já tem uma conta?" : "Novo no Yugen?"} <button onClick={() => switchTo(type === "join" ? "login" : "join")}>{type === "join" ? "Entrar" : "Criar uma"}</button></p></section></div>;
}
